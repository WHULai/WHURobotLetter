    ...
    #__apply_smallcaps(
      {
        ...
        name + " · " + linguify("letter", from: lang_data)
      },
      use-smallcaps,
    )
  ][
...
#let __resume_footer(author, language, lang_data, date, use-smallcaps: true, resume-title: none) = {
  set text(fill: gray, size: 8pt)
  __justify_align_3[
    #__apply_smallcaps(date, use-smallcaps)
  ][
    #__apply_smallcaps(
      {
        let name = __format_author_name(author, language)
        let title-text = if resume-title != none {
          resume-title
        } else {
...
/// - resume-title (str | none): Custom title for the resume (displayed in PDF metadata and footer). If not set, uses the translated "Resume" text.
...
#let default-closing(lang-data, attachment-text: none) = {
  if attachment-text != none {
    align(bottom)[
      #text(weight: "light", style: "italic")[
        #linguify("attached", from: lang-data)#sym.colon #attachment-text
      ]
    ]
  }
}
...
/// This letter template is designed to be used with the resume template.
...
/// - profile-picture (image): The profile picture of the author. This will be cropped to a circle and should be square in nature.
/// - date (datetime): The date the letter was created. This will default to the current date.
...
/// - description (str | none): The PDF description
/// - keywords (array | str): The PDF keywords
/// - body (content): The body of the letter
#let letter(
  author: (:),
  profile-picture: image,
  date: datetime.today().display("[month repr:long] [day], [year]"),
  ...
  let desc = if description == none {
    (
      lflib._linguify("letter", lang: language, from: lang_data).ok
        + " "
        + author.firstname
        + " "
        + author.lastname
  ...
  show: body => context {
    set document(
      author: author.firstname + " " + author.lastname,
      title: lflib
        ._linguify("letter", lang: language, from: lang_data)
        .ok,
      description: desc,
      keywords: keywords,
    )
  ...
  set page(
    paper: paper-size,
    margin: (left: 15mm, right: 15mm, top: 10mm, bottom: 10mm),
    footer: if show-footer [#__letter_footer(
      author,
      language,
      date,
      lang_data,
...
/// Recipient information for the letter. Formats the recipient's address and details properly.
/// - recipient-info (content): The information of the recipient including the name, the target (attention to), street address, and city
...
#let recipient-info(
  ...
  pad(top: 1.5em, bottom: 1.5em)[
    #__justify_align[
      #text(weight: "bold", size: 12pt)[#recipient-info.target]
    ][
    ...
    #pad(top: 0.65em, bottom: 0.65em)[
      #text(weight: "regular", fill: color-gray, size: 9pt)[
        #__apply_smallcaps(recipient-info.name, use-smallcaps) \
        #recipient-info.street-address \
        #recipient-info.city \
      ]
    ]
  ]
...
/// Letter heading for the addressee with optional subject.
/// - subject (string): Optional subject line for the letter
/// - addressee (string): The person you are addressing the letter to
/// - dear (string): optional field for redefining the "dear" variable
#let letter-heading(addressee: "", dear: "", subject: none) = {
  let lang_data = toml("lang.toml")

  if subject != none {
    underline(evade: false, stroke: 0.5pt, offset: 0.3em)[
      #text(weight: "bold", size: 12pt)[#subject]
    ]
...
/// Letter content paragraph. This is the main content of the letter.
/// - content (content): The content of the letter
#let letter-content(content) = {
  pad(top: 1em, bottom: 1em)[
